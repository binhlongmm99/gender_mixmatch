...
DOMAIN_LIST = ["domain_hawkice_damdev", "domain_hawkice_longphong"]
...
def main():
    ...
    train_set, train_unlabeled_set, test_set = dataset.get_gender_data_from_all_domains(
                                                                INPUT_DIR_PATH, DOMAIN_LIST, 
                                                                transform_train=transform_train, 
                                                                transform_val=transform_val)

    labeled_trainloader = data.DataLoader(train_set['train'], batch_size=args.batch_size, 
                                            shuffle=True, num_workers=0, drop_last=True)
    
    val_loader = data.DataLoader(train_set['val'], batch_size=args.batch_size, 
    ...
    for domain in DOMAIN_LIST:
        test_loader[domain] = data.DataLoader(test_set[domain], batch_size=args.batch_size, 
                                  shuffle=False, num_workers=0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        test_loss, test_acc = validate_test(test_loader, ema_model, criterion, 
        ...
        for i, domain in enumerate(DOMAIN_LIST):
            tag = 'losses/test_loss_' + domain
            writer.add_scalar(tag, test_loss[i], step)

        writer.add_scalar('accuracy/train_acc', train_acc, step)
        writer.add_scalar('accuracy/val_acc', val_acc, step)
        for i, domain in enumerate(DOMAIN_LIST):
            tag = 'accuracy/test_acc_' + domain
            writer.add_scalar(tag, test_acc[i], step)
...
def validate_test(valloader, model, criterion, epoch, use_cuda, mode):
    loss_all_domains = []
    acc_all_domains = []
    for domain in DOMAIN_LIST:
        batch_time = AverageMeter()
        ...
        bar = Bar(f'{mode + " " + domain}', max=len(valloader[domain]))
        
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(valloader[domain]):
                # measure data loading time
                data_time.update(time.time() - end)
                ...
                bar.next()
            bar.finish()
        loss_all_domains.append(losses.avg)
        acc_all_domains.append(top1.avg)
    return loss_all_domains, acc_all_domains
